...
import { Requisition, User, Role, PaymentMethod, RequisitionStatus, Department, Section, Church } from '../types';
...
const RequisitionDetail: React.FC<RequisitionDetailProps> = ({ requisitionId, currentUser, navigate }) => {
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const paymentDetails = {
            ...
            proofFile: paymentProof ? { name: paymentProof.name, url: '#' } : null,
  ...
  const canTakeAction = () => {
    ...
    const { status, approvals, requestedById } = requisition;
    const { role, id, departmentId, sectionId } = currentUser;

    const hasAlreadyActed = approvals.some(a => a.approverId === id);
    if (hasAlreadyActed) return false;
    
    if (status === RequisitionStatus.PENDING) {
        const requester = churchData.users.find(u => u.id === requestedById);
        if (!requester) return false;
        if (requester.role !== Role.DEPARTMENT_HEAD && role === Role.DEPARTMENT_HEAD && departmentId === requisition.departmentId) return true;
        if (requester.role === Role.DEPARTMENT_HEAD && role === Role.SECTION_PRESIDENT && sectionId === requisition.sectionId) return true;
    }
    
    if (status === RequisitionStatus.APPROVED_BY_DEPT_HEAD && role === Role.SECTION_PRESIDENT && sectionId === requisition.sectionId) return true;

    return false;
  };
  
  const canDisburse = () => requisition?.status === RequisitionStatus.APPROVED_BY_SECTION_PRESIDENT && currentUser.role === Role.FINANCE && currentUser.sectionId === requisition.sectionId;
  const canUploadReceipt = () => (requisition?.status === RequisitionStatus.AWAITING_RECEIPT || requisition?.status === RequisitionStatus.RECEIPT_CORRECTION_REQUESTED) && requisition.requestedById === currentUser.id;
  const canVerifyReceipt = () => requisition?.status === RequisitionStatus.PENDING_FINANCE_VERIFICATION && currentUser.role === Role.FINANCE && currentUser.sectionId === requisition.sectionId;
  const canEditAndResubmit = () => requisition?.status === RequisitionStatus.CHANGES_REQUESTED && requisition.requestedById === currentUser.id;
  ...
  const getUserName = (id: number) => churchData.users.find(u => u.id === id)?.name || 'Unknown User';
  const getDepartmentName = (id: number) => churchData.church?.sections.flatMap(s => s.departments).find(d => d.id === id)?.name;
  const getSectionName = (id: number) => churchData.church?.sections.find(s => s.id === id)?.name;
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <div className="mt-4 border-t pt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><p className="font-medium text-slate-600">Amount</p><p className="text-lg font-bold text-slate-800">₦{requisition.amountRequested.toLocaleString()}</p></div>
            <div><p className="font-medium text-slate-600">Requested By</p><p className="text-slate-800">{getUserName(requisition.requestedById)}</p></div>
            <div><p className="font-medium text-slate-600">Section</p><p className="text-slate-800">{getSectionName(requisition.sectionId) || 'N/A'}</p></div>
            <div><p className="font-medium text-slate-600">Department</p><p className="text-slate-800">{getDepartmentName(requisition.departmentId) || 'N/A'}</p></div>
            <div><p className="font-medium text-slate-600">Category</p><p className="text-slate-800">{requisition.category}</p></div>
            <div><p className="font-medium text-slate-600">Date Created</p><p className="text-slate-800">{new Date(requisition.createdAt).toLocaleString()}</p></div>
            <div><p className="font-medium text-slate-600">Date Needed</p><p className="text-slate-800">{new Date(requisition.dateNeeded).toLocaleDateString()}</p></div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
            ...
            <ul className="space-y-4">
                {requisition.activityLog.slice().reverse().map(log => (
                    <li key={log.id} className="flex gap-3">
                        <div className="flex-shrink-0 h-8 w-8 bg-slate-200 rounded-full flex items-center justify-center text-xs font-bold text-slate-600">{getUserName(log.userId).substring(0,2)}</div>
                        <div>
                            <p className="text-sm"><span className="font-medium">{getUserName(log.userId)}</span> {log.action}</p>
             ...
             {requisition.payment && (
                <Card>
                    ...
                    <div className="space-y-2 text-sm">
                        <p><span className="font-medium">Amount:</span> ₦{requisition.payment.amountPaid.toLocaleString()}</p>
                        <p><span className="font-medium">Method:</span> {requisition.payment.paymentMethod}</p>
                        <p><span className="font-medium">Date:</span> {new Date(requisition.payment.paymentDate).toLocaleDateString()}</p>
                        <p><span className="font-medium">Reference:</span> {requisition.payment.referenceNumber || 'N/A'}</p>
                        {requisition.payment.proofFile && <p className="text-blue-600"><a href="#">View Proof</a></p>}
                    </div>
                </Card>
            )}
             {requisition.finalReceipt && (
                <Card>
                    <h3 className="font-semibold text-slate-800 mb-2">Final Receipt</h3>
                     <p className="text-sm text-blue-600"><a href="#" className="underline hover:no-underline">{requisition.finalReceipt.name}</a></p>
                </Card>
            )}
        </div>
      </div>
