...
import { User, Section, Role, Church } from '../types';
...
const BillingPanel: React.FC<{ church: Church | null, currentUserId: number }> = ({ church, currentUserId }) => {
    ...
    const isExpired = new Date(church.subscriptionEndsAt) < new Date();
        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                <div className="bg-slate-50 p-4 rounded-lg">
                    ...
                    <p className="text-lg font-bold text-slate-800">{church.subscriptionStatus}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-lg">
                    <p className="font-medium text-slate-500">Expires On</p>
                    <p className={`text-lg font-bold ${isExpired ? 'text-red-600' : 'text-slate-800'}`}>
                        {new Date(church.subscriptionEndsAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </p>
                </div>
            </div>
            <div className="mt-8">
...
const AdminPanel: React.FC<{ currentUser: User; church: Church | null }> = ({ currentUser, church }) => {
    ...
    useEffect(() => {
        fetchData();
    }, [currentUser.id, currentUser.churchId]);
    ...
    const handleCreateSection = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!newSectionName || !currentUser.churchId) return;
        setIsSubmitting(true);
        try {
            await apiClient.createSection(currentUser.churchId, newSectionName, currentUser.id);
            setNewSectionName('');
            await fetchData();
        } catch (error) {
    ...
    const handleCreateUser = async (e: React.FormEvent) => {
        ...
        if (!newUserName || !newUserEmail || !newUserRole || !newUserSectionId || !currentUser.churchId || (needsDepartment && !newUserDepartmentId)) {
        ...
        try {
            const userData: Omit<User, 'id'> = {
                ...
                role: newUserRole,
                sectionId: parseInt(newUserSectionId),
                departmentId: needsDepartment ? parseInt(newUserDepartmentId) : null,
    ...
    const availableDepartments = newUserSectionId
        ? sections.find(s => s.id === parseInt(newUserSectionId))?.departments || []
        : [];
