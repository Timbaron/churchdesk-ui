...
import { Church, SubscriptionStatus, PlatformActivity } from '../types';
...
const ExtendSubscriptionModal: React.FC<{
    ...
    extendSubscription: (churchId: string, months: number) => Promise<Church>;
    isSubmitting: boolean;
}> = ({ church, onClose, onSuccess, extendSubscription, isSubmitting }) => {
...
const AppOwnerDashboard: React.FC = () => {
    ...
    const subscriptionChartData = useMemo(() => {
        ...
        return Object.entries(platformData.subscription_status_counts).map(([name, value]) => ({ name, value }));
    }, [platformData]);

    const requisitionChartData = useMemo(() => {
        if (!platformData?.requisition_status_counts) return [];
        return Object.entries(platformData.requisition_status_counts).map(([name, value]) => ({ name, value }));
    }, [platformData]);
        ...
        <div className="bg-slate-100 min-h-screen">
            ...
            <main className="p-4 md:p-8 space-y-8">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                    ...
                    <StatCard title="Total Users" value={platformData.total_users} />
                    <StatCard title="Total Requisitions" value={platformData.total_requisitions} />
                    <StatCard title="Total Value Requested" value={`₦${(platformData.total_amount_requested / 1000000).toFixed(1)}M`} />
                    <StatCard title="Active Subscriptions" value={platformData.subscription_status_counts?.Active || 0} />
                    <StatCard title="Trial Accounts" value={platformData.subscription_status_counts?.Trial || 0} />
                </div>
                                ...
                                <tbody>
                                    {filteredChurches.map(church => (
                                        <tr key={church.id} className="bg-white border-b hover:bg-slate-50">
                                            ...
                                            <td className="px-6 py-4"><SubscriptionBadge status={church.subscription_status} /></td>
                                            <td className="px-6 py-4">{new Date(church.subscription_ends_at).toLocaleDateString()}</td>
